...
async function scrapeDynamicNews(url) {
  ...
  const allResultDatas = [];
  let contentsHeaderSet = new Set();

  for await (const startTime of setInterval(3000)) {
    ...
    contentsHeaderSet.add(data.contentsHeader);

    // 중복데이터 검열
    if (!contentsHeaderSet.has(data.contentsHeader)) {
      allResultDatas.push(data);
    }
...
/************************************************************************************* */
function scrapeDynamicDatas(url) {
  try {
    scrapeDynamicImages(url);
    scrapeDynamicNews(url);
  } catch (error) {
    console.error(error);
  }
}

scrapeDynamicDatas(URL);
